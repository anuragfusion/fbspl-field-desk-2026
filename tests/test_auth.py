...
def make_user(username, password, role="field", disabled=False):
    with db() as conn:
        ...
        conn.execute(
            ...
            " disabled_at, created_at, updated_at) VALUES (%s,%s,%s,%s,%s,%s,%s,%s)",
...
class AuthTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        with db() as conn:
            # Against a persistent Postgres (unlike the old per-run sqlite tempfile),
            # rows from other test modules' runs can still be sitting in the DB, so
            # clear every table that can hold a foreign key into users first.
            for t in ("op_log", "update_receipts", "leads", "documents", "updates",
                      "client_pocs", "clients", "event_members", "events",
    ...
    def test_wrong_password_is_401_and_increments_failed_attempts(self):
        ...
        with db() as conn:
            row = conn.execute("SELECT failed_attempts FROM users WHERE id=%s", (uid,)).fetchone()
    ...
    def test_stale_failures_outside_the_window_do_not_accumulate(self):
        ...
        with db() as conn:
            conn.execute("UPDATE users SET failed_attempts=9, last_failed_at=%s WHERE id=%s",
                         (old, uid))
        self.login("priya", "wrong")
        with db() as conn:
            row = conn.execute("SELECT failed_attempts FROM users WHERE id=%s", (uid,)).fetchone()
    ...
    def test_disabled_account_cannot_log_in_and_its_session_stops_working(self):
        ...
        with db() as conn:
            conn.execute("UPDATE users SET disabled_at=%s WHERE id=%s", (now_iso(), uid))
    ...
    def test_expired_session_is_rejected(self):
        ...
        token = self.login("priya", "correct-horse").json()["token"]
        with db() as conn:
            conn.execute("UPDATE sessions SET expires_at=%s",
